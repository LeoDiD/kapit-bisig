...
export default function DistributionPageClient() {
  ...
  const handleCreate = async (payload: CreateDistributionPayload) => {
    try {
      setError(null)
      const scheduledDate = new Date(payload.scheduled)
      const fallbackEndsAt = new Date(scheduledDate.getTime() + 4 * 60 * 60 * 1000).toISOString()
      await api.createDistribution({
        ...
        endsAt: payload.endsAt || fallbackEndsAt,
        notes: payload.notes,
      }, {
